...
public class University
{
    ...
    protected ArrayList<Cohort> sortMethod(ArrayList<Cohort> list, int block_size, boolean ascending, String attr) {
        ...
        for (int outerIndex = 1; outerIndex < listSize; outerIndex++) {
            Cohort targetCohort = list.get(outerIndex);
            int compareIndex = outerIndex - 1;

            while (compareIndex >= 0) {
                Cohort currentCohort = list.get(compareIndex);
                int compareResult;

                if (attr.equalsIgnoreCase("name")) {
                    compareResult = currentCohort.getModule().getName().compareTo(targetCohort.getModule().getName());
                } else {
                    compareResult = currentCohort.getModule().getCode() - targetCohort.getModule().getCode();
                }
                ...
                if (needsShift) {
                    list.set(compareIndex + 1, list.get(compareIndex));
                    compareIndex--;
                } else {
                    break;
                }
            }
            list.set(compareIndex + 1, targetCohort);
        }
        return list;
    }
    }
